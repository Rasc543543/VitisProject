...
static void cfo_estimator(hls::stream<cfo_corr_t> &in_corr, hls::stream<cfo_t> &out_phase_inc);
static void cfo_correction(hls::stream<internal_axis_t> &in_payload, hls::stream<cfo_t> &in_phase_inc, hls::stream<axis_word> &out_axis);
...
#pragma HLS DATAFLOW
  ...
  hls::stream<cfo_t>           stream_phase_inc("stream_phase_inc");
...
#pragma HLS STREAM variable = stream_phase_inc   depth = 4   impl = fifo
  ...
  cfo_estimator(stream_locked_corr, stream_phase_inc);
  cfo_correction(stream_payload, stream_phase_inc, output_stream);
}
...
#pragma HLS BIND_OP variable = target_thresh op = mul impl = dsp
    ...
    // 無條件更新歷史輸入樣本，確保在 STATE_OUTPUT_PAYLOAD 也能依序推移 x_n_m2
...
static void cfo_correction(hls::stream<internal_axis_t> &in_payload, hls::stream<cfo_t> &in_phase_inc, hls::stream<axis_word> &out_axis) {
  static ap_fixed<16, 1, AP_TRN, AP_WRAP> phase_acc  = 0.0f;
  static ap_fixed<16, 1, AP_TRN, AP_WRAP> phase_inc_fixed = 0.0f;
  static bool  new_symbol = true;
...
#pragma HLS PIPELINE II = 1
    ...
    if (new_symbol) {
      ...
      ap_fixed<16, 1, AP_TRN, AP_WRAP> phase_inc_fixed_tmp = (ap_fixed<16, 1, AP_TRN, AP_WRAP>)(phaseInc * 0.3183098861837907f);
      phase_inc_fixed = phase_inc_fixed_tmp;
      phase_acc = (ap_fixed<16, 1, AP_TRN, AP_WRAP>)(phase_inc_fixed_tmp * (ap_fixed<16, 9>)255);
      new_symbol = false;
    }
    ...
    ap_uint<8> cos_idx = idx + 64; // +90 degrees phase shift
...
#pragma HLS BIND_OP variable = mul_q_cos op = mul impl = dsp
    ...
    out_pkt.keep = 0xF; out_pkt.strb = 0xF; out_pkt.last = pkt.last ? 1 : 0;
    out_axis.write(out_pkt);

    if (pkt.last) new_symbol = true;
  }
}