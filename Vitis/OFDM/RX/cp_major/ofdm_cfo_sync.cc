...
static void calc_power_and_corr2(hls::stream<complex_data_t> &in_data, hls::stream<bool> &in_clear_gate,
  hls::stream<mult_t> &out_power, hls::stream<complex_mult_t> &out_corr1, hls::stream<complex_mult_t> &out_corr2,
  hls::stream<complex_mult_t> &out_corr3, hls::stream<complex_data_t> &out_data_payload);
static void sync_search(hls::stream<mult_t> &in_power, hls::stream<complex_mult_t> &in_corr1,
  hls::stream<complex_mult_t> &in_corr2, hls::stream<complex_mult_t> &in_corr3, acc_t threshold_config,
  hls::stream<cfo_corr_t> &out_corr, hls::stream<bool> &out_sync_flag);
static void payload_gate(hls::stream<complex_data_t> &in_data_payload, hls::stream<bool> &in_sync_flag,
  hls::stream<internal_axis_t> &out_payload);
static void combined_correction(hls::stream<cfo_corr_t> &in_corr, hls::stream<internal_axis_t> &in_payload,
...
#pragma HLS DATAFLOW
  ...
  hls::stream<complex_data_t>  stream_data_payload("stream_data_payload");
  ...
  hls::stream<bool>            stream_sync_flag("stream_sync_flag");

#pragma HLS STREAM variable = stream_data         depth = 256
#pragma HLS STREAM variable = stream_clear_gate   depth = 256
#pragma HLS STREAM variable = stream_data_payload depth = 256
...
#pragma HLS STREAM variable = stream_payload      depth = 1024 impl = bram
#pragma HLS STREAM variable = stream_locked_corr  depth = 4    impl = fifo
#pragma HLS STREAM variable = stream_sync_flag    depth = 256  impl = fifo

  unpack_axis(input_stream, stream_data, stream_clear_gate);
  calc_power_and_corr2(stream_data, stream_clear_gate, stream_power, stream_corr1, stream_corr2, stream_corr3,
    stream_data_payload);
  sync_search(stream_power, stream_corr1, stream_corr2, stream_corr3, INTERNAL_THRESHOLD, stream_locked_corr,
    stream_sync_flag);
  payload_gate(stream_data_payload, stream_sync_flag, stream_payload);
...
#pragma HLS BIND_STORAGE variable = delay_line_128 type = ram_2p impl = lutram
  ...
  // 2-tap: x_payload = x[k-2050]
  // j_fire = k_peak + 3 = 2303 + 3 = 2306
  // x[2306-2050] = x[256] = Data[0]
  static complex_data_t delay_payload[2];
#pragma HLS ARRAY_PARTITION variable = delay_payload complete

  while (true) {
#pragma HLS PIPELINE II = 1
#ifndef __SYNTHESIS__
    if (in_data.empty() || in_clear_gate.empty()) break;
#endif
    ...
    delay_payload[0] = x_n_2048;

    mult_t sq_r  = x_n.real() * x_n.real();
    mult_t sq_i  = x_n.imag() * x_n.imag();
...
#pragma HLS BIND_OP variable = sq_r op = mul impl = dsp
#pragma HLS BIND_OP variable = sq_i op = mul impl = dsp
...
#pragma HLS BIND_STORAGE variable = delay_corr3 type = ram_2p impl = lutram
  ...
  static sq_metric_t   thresh_m2 = 0; static sq_metric_t   thresh_m3 = 0;
...
#pragma HLS PIPELINE II = 1
    ...
    // Always accumulate regardless of post_detection state
    {
      ...
      acc_corr1 = complex_acc_t(acc_corr1.real() + c1_n.real() - c1_to_sub.real(),
                                acc_corr1.imag() + c1_n.imag() - c1_to_sub.imag());
      acc_corr2 = complex_acc_t(acc_corr2.real() + c2_n.real() - c2_to_sub.real(),
                                acc_corr2.imag() + c2_n.imag() - c2_to_sub.imag());
      acc_corr3 = complex_acc_t(acc_corr3.real() + c3_n.real() - c3_to_sub.real(),
                                acc_corr3.imag() + c3_n.imag() - c3_to_sub.imag());

      if (search_warmup < SUB_CP_LEN) search_warmup++;
    }

    sq_metric_t r_sq = acc_corr1.real() * acc_corr1.real();
    sq_metric_t i_sq = acc_corr1.imag() * acc_corr1.imag();
    sq_metric_t p_sq = acc_pwr * acc_pwr;
...
#pragma HLS BIND_OP variable = p_sq op = mul impl = dsp
    ...
    metric_m0 = r_sq + i_sq;
    sq_metric_t target_thresh = threshold_config * p_sq;
#pragma HLS BIND_OP variable = target_thresh op = mul impl = dsp
    ...
    if (post_detection) {
      if (post_counter == (ap_uint<12>)(FFT_LEN - 1)) {
        ...
        post_counter   = 0;
        // Reset metric shift registers only; accumulator state preserved
        metric_m0 = 0; metric_m1 = 0; metric_m2 = 0; metric_m3 = 0;
        thresh_m2 = 0; thresh_m3 = 0;
        corr1_m1 = complex_acc_t(0,0); corr2_m1 = complex_acc_t(0,0); corr3_m1 = complex_acc_t(0,0);
        corr1_m2 = complex_acc_t(0,0); corr2_m2 = complex_acc_t(0,0); corr3_m2 = complex_acc_t(0,0);
        corr1_m3 = complex_acc_t(0,0); corr2_m3 = complex_acc_t(0,0); corr3_m3 = complex_acc_t(0,0);
      } else {
        post_counter++;
...
#pragma HLS PIPELINE II = 1
#ifndef __SYNTHESIS__
    if (in_data_payload.empty() || in_sync_flag.empty()) break;
#endif
    complex_data_t x       = in_data_payload.read();
    bool           is_peak = in_sync_flag.read();

    if (!outputting && is_peak) {
      outputting = true;
      cnt        = 0;
    }
    ...
    if (outputting) {
      internal_axis_t pkt;
      pkt.data.range(15, 0)  = x.real().range(15, 0);
      pkt.data.range(31, 16) = x.imag().range(15, 0);
      pkt.last = (cnt == (ap_uint<12>)(FFT_LEN - 1));
      out_payload.write(pkt);

      if (cnt == (ap_uint<12>)(FFT_LEN - 1)) {
        outputting = false;
        cnt        = 0;
      } else {
        cnt++;
...
static void combined_correction(hls::stream<cfo_corr_t> &in_corr, hls::stream<internal_axis_t> &in_payload,
  hls::stream<axis_word> &out_axis) {
  // outer while: blocking read on in_corr (one per OFDM symbol)
  // atan2f runs here at non-pipelined rate, avoids multi-FIFO conditional-read deadlock
  // inner for: PIPELINE II=1 consumes in_payload
  while (true) {
#ifndef __SYNTHESIS__
