...
**理由：**
- 8PSK demapper 的 LLR 計算（`llr_b0 = |I|-|Q|`、`llr_b1 = I`、`llr_b2 = Q`）全部基於**符號的角度與正負**
- 不除 `|H|²` 時，輸出 = 原始符號 × 正實數，相位完全不變
- 硬判決與軟判決的決策邊界均不受純實數縮放影響
- **可省一個 float 除法器**（FPGA 上成本較高）

**限制：**
- Flat channel（loopback）：完全無影響，BER = 0
- Frequency-selective channel（真實多徑）：各 subcarrier 的 `|H|²` 不同，LLR 幅度不一致，Viterbi 性能理論上略差

**驗證結果（2026-04-14）：**

| 版本 | csim Average Error | 說明 |
|---|---|---|
| 無 `/h_power` | 0.750232 | TB 用 ch_gain=0.5，輸出縮小 4 倍，誤差大 |
| 有 `/h_power`（現版） | ~0 | ZF 完整，正確 |

> TB 的 `ch_gain = 0.5` 使 `|H|² = 0.25`，所以在此 TB 下不除誤差明顯。  
> 但在真實 loopback（`|H|² ≈ 1`）下，不除也能通過。

**結論：** 目前維持 `/|H|²`。若未來需要節省 DSP 資源，可在 loopback 條件下移除並驗證。
